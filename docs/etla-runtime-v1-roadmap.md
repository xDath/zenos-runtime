...
- persisted model-call lifecycle events, per-turn fresh/cache/output accounting, SSE `activity` delivery, a SQLite watcher, safe model-slot setup wizard, a realtime 9Router tracker with Host/Worker/Verifier/Boss history, and deterministic Host execution receipts that distinguish Host planning from final synthesis;
- native two-phase Hermes Gateway middleware with Host-led orchestration: direct chat begins with the Hermes Host, while serious turns first invoke a bounded Host Planner that understands the goal, chooses the minimum sufficient Worker/Verifier/Boss delegation, supplies acceptance criteria, and then returns control to the Host for final synthesis; deterministic routing remains a safety baseline, Worker cannot lead a turn when Host planning fails, Boss remains mandatory only for explicit requests or critical safety floors, and Verifier/Boss postflight still gates delivery when required;
